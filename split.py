...
if(len(sys.argv) != 4):
        sys.exit("usage: split_fasta.py PATH MULTI_FASTA_FILE N_CHUNKS")


path=sys.argv[1]  # path to fasta file
file=sys.argv[2] # fasta file
chunks = sys.argv[3] # number of chunks
ffile = path + "/" + file
...
for i, batch in enumerate(batch_iterator(records, chunksize)):
    if i == 0:
        filename = path + "/chunk_%i.fasta" % (i + 1)
